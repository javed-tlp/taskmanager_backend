...
exports.verifyToken = (req, res, next) => {
    //.log("Step 1: Entered verifyToken middleware");

    // Get the token from the authorization header
    const token = req.headers['authorization'];  
    //.log("Step 2: Received Authorization Header:", req.headers['authorization']);

    if (!token) {
        //.error("Step 3: Token not provided in the header");
        return res.status(403).json({ error: 'No token provided' });
    }

    // Remove the 'Bearer ' prefix if it exists
    const cleanToken = token.startsWith('Bearer ') ? token.slice(7) : token;
    //.log("Step 4: Cleaned Token (without Bearer prefix):", cleanToken);  // Log cleaned token

    // Verify the JWT
    jwt.verify(cleanToken, JWT_SECRET, (err, decoded) => {
        if (err) {
            //.error('Step 5: JWT Verification Error:', err.message);  // Log detailed error
